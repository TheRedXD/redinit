...
namespace CPPConanCMake {
    ...
    bool satisfiesDepends() {
        ...
        if (std::system("which cmake > /dev/null") != 0) {
            std::cout << COLOR_RED << head << "Error: CMake is not installed." << COLOR_RESET << std::endl;
            return false;
        }
        if (std::system("which make > /dev/null") != 0) {
            std::cout << COLOR_RED << head << "Error: Make is not installed." << COLOR_RESET << std::endl;
            return false;
        }
    }
    ...
    int create(std::string projectName) {
        ...
        createFile("rel.sh", "#!/bin/bash\n\nconan install . -sbuild_type=Release -of=conan/build --build=missing\nrm -rf build\nmkdir build\ncmake -B ./build -DCMAKE_TOOLCHAIN_FILE=conan/build/conan_toolchain.cmake -DCMAKE_BUILD_TYPE=Release");
        createFile("dbg.sh", "#!/bin/bash\n\nconan install . -sbuild_type=Debug -of=conan/deb --build=missing\nrm -rf build\nmkdir build\ncmake -B ./build -DCMAKE_TOOLCHAIN_FILE=conan/deb/conan_toolchain.cmake -DCMAKE_BUILD_TYPE=Debug");
        createFile("mk.sh", "#!/bin/bash\n\ncd ./build\nmake\ncd ..");
        std::system("chmod +x rel.sh");
        std::system("chmod +x dbg.sh");
        std::system("chmod +x mk.sh");
        ...
        std::cout << COLOR_GREEN << head << "Building project" << COLOR_RESET << std::endl;
        std::system("./rel.sh");
        std::system("./mk.sh");
        std::cout << COLOR_GREEN << head << "Built project" << COLOR_RESET << std::endl;
        std::cout << COLOR_GREEN << head << "Done" << COLOR_RESET << std::endl;
        std::cout << std::endl;
        std::cout << COLOR_WHITE << head << "INSTRUCTIONS" << COLOR_RESET << std::endl;
        std::cout << COLOR_WHITE << head << "- Run ./rel.sh to build in Release mode" << COLOR_RESET << std::endl;
        std::cout << COLOR_WHITE << head << "- Run ./dbg.sh to build in Debug mode" << COLOR_RESET << std::endl;
        std::cout << COLOR_WHITE << head << "- Run ./mk.sh to run make in build directory" << COLOR_RESET << std::endl;
        std::cout << COLOR_WHITE << head << "- Run ./build/" << projectName << " to run the program" << COLOR_RESET << std::endl;
        return 0;
    }
}
